...
export default function ScriptOutput({ output, onUpdate, onRegenerate }: ScriptOutputProps) {
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      doc.text('Viral Script Strategy', margin, yPos);
      yPos += 15;

      // Hook
      doc.setFontSize(14);
      doc.setTextColor(30, 41, 59); // Slate-800
      doc.text('THE HOOK', margin, yPos);
      yPos += 7;
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(12);
      const hookLines = doc.splitTextToSize(`"${output.hook}"`, pageWidth - (margin * 2));
      doc.text(hookLines, margin, yPos);
      yPos += (hookLines.length * 7) + 10;

      // Script
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('FULL SCRIPT', margin, yPos);
      yPos += 7;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(11);
      const scriptLines = doc.splitTextToSize(output.script, pageWidth - (margin * 2));
      
      // Check if script needs new page
      if (yPos + (scriptLines.length * 6) > doc.internal.pageSize.getHeight() - 20) {
        doc.addPage();
        yPos = 20;
      }
      doc.text(scriptLines, margin, yPos);
      yPos += (scriptLines.length * 6) + 10;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('VIRAL TITLES', margin, yPos);
      yPos += 7;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(11);
      output.titles.forEach((title, i) => {
        const titleText = `${i + 1}. ${title}`;
        const lines = doc.splitTextToSize(titleText, pageWidth - (margin * 2));
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(11);
      const ctaLines = doc.splitTextToSize(output.cta, pageWidth - (margin * 2));
      doc.text(ctaLines, margin, yPos);
      yPos += (ctaLines.length * 6) + 10;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('SEO & HASHTAGS', margin, yPos);
      yPos += 7;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      doc.text(`Keywords: ${output.keywords.join(', ')}`, margin, yPos);
      yPos += 7;
      doc.text(`Hashtags: ${output.hashtags.join(' ')}`, margin, yPos);
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Generated by Viral Script Strategist - Page ${i} of ${pageCount}`, pageWidth / 2, doc.internal.pageSize.getHeight() - 10, { align: 'center' });
      }

      doc.save(`Viral_Script_${output.hook.slice(0, 20).replace(/\s+/g, '_')}.pdf`);
    } catch (error) {
        ...
        <button
          ...
          className={`flex items-center gap-2 px-8 py-4 rounded-2xl font-black transition-all shadow-xl ${
            isExporting 
              ...
              : 'bg-white text-indigo-600 border-2 border-indigo-600 hover:bg-indigo-50 shadow-indigo-500/10'
          }`}
        >
          {isExporting ? (
